...
class SecureLauncher {
    ...
    setupIPC() {
        ...
        // Open terminal
        ipcMain.handle('open-terminal', async (event, directory) => {
            return await this.openTerminal(directory);
        });
...
`;
    ...
    async loginToVercel() {
        try {
            ...
            this.log('Opening Vercel login in your browser...');
            this.log('Please complete the login process in your browser, then return here.');
            ...
            return new Promise((resolve, reject) => {
                ...
                let loginTimeout = setTimeout(() => {
                    ...
                    this.log('Login process timed out after 2 minutes');
                    resolve({ success: false, error: 'Login process timed out. Please try manual login.' });
                ...
                loginProcess.on('close', async (code) => {
                    ...
                    try {
                        if (code === 0) {
                            ...
                            process.chdir(originalDir);
                            this.log('Vercel login was cancelled or failed');
                            resolve({ success: false, error: 'Login process was cancelled. You can try manual login.' });
                        }
                    } catch (error) {
                        process.chdir(originalDir);
                        this.log(`Login verification failed: ${error.message}`, 'error');
                        resolve({ success: false, error: 'Login completed but verification failed. Try manual login or restart the launcher.' });
                    }
                });
                ...
                loginProcess.on('error', (error) => {
                    clearTimeout(loginTimeout);
                    process.chdir(originalDir);
                    this.log(`Login process error: ${error.message}`, 'error');
    ...
    async openTerminal(directory) {
        try {
            ...
            this.log(`Terminal opened in ${directory}`);
            return { success: true };
        } catch (error) {
            this.log(`Failed to open terminal: ${error.message}`, 'error');
            return { success: false, error: error.message };
        }
    }
